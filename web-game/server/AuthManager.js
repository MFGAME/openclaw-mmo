...
import { MessageType } from './WebSocketServer';
...
export class AuthManager {
     ...
     * 启动会话清理定时器
     */
    startSessionCleanup(interval = 60 * 60 * 1000) {
     ...
     * 集成 WebSocket 服务器
     */
    integrateWithWebSocketServer(wsServer) {
        this.wsServer = wsServer;
        // 注册认证消息处理器
        wsServer.onMessage(MessageType.AUTH, async (client, message) => {
    ...
    getStats() {
        const now = Date.now();
        const activeSessions = Array.from(this.sessions.values()).filter(s => s.expiresAt > now && now - s.lastActive < 30 * 60 * 1000 // 30 分钟内活跃
        );
        return {
