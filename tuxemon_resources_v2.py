...
def setup_tuxemon_routes(app: FastAPI):
    """设置Tuxemon资源路由"""
    ...
    async def get_monster_sprite(monster_slug: str):
        ...
        if os.path.exists(sprite_file):
            ...
            raise HTTPException(status_code=404, detail=f"Sprite not found: {monster_slug}")
    ...
    async def get_monster_sprite_frame(monster_slug: str, frame_num: int = 1):
        ...
        try:
            ...
            # 计算帧位置（取站立帧）
            frame_x = frame_width * min(frame_num - 1, 2)
            frame_y = 0
            ...
            print(f"Error processing sprite {monster_slug}: {e}")
    ...
    async def list_sprites():
        ...
        if not os.path.exists(SPRITE_DIR):
            return {"sprites": [], "error": "Sprite directory not found"}
        
        sprites = [f.replace('-sheet.png', '') for f in os.listdir(SPRITE_DIR) 
...
print(f"[OK] Tuxemon resource routes registered")
print(f"[OK] Sprite directory: {SPRITE_DIR}")
if os.path.exists(SPRITE_DIR):
    sprites = [f for f in os.listdir(SPRITE_DIR) if f.endswith('-sheet.png')]
    print(f"[OK] Available sprites: {len(sprites)}")
